...
//  	pthread_mutex_lock(&v->sock_lock);
  ...
  //		pthread_mutex_unlock(&v->sock_lock);
  	...
  	curl = v->curl;
  	if (!v->sockbuf) {
  		v->sockbuf = (char*) calloc(RBUFSIZE, 1);
  		v->sockbuf_size = RBUFSIZE;
  	}
    v->sockbuf[0] = '\0';
    pthread_mutex_unlock(&v->sock_lock);
    	if (url != v->url) {
    	...
    	free(v->curl_url);
    	v->curl_url = (char*) malloc(strlen(url));
    	sprintf(v->curl_url, "http%s", strstr(url, "://"));

    	if (opt_protocol)
    		curl_easy_setopt(curl, CURLOPT_VERBOSE, 1);
    	curl_easy_setopt(curl, CURLOPT_URL, v->curl_url);
    	curl_easy_setopt(curl, CURLOPT_FRESH_CONNECT, 1);
    	curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 30);
    	curl_easy_setopt(curl, CURLOPT_ERRORBUFFER, v->curl_err_str);
    	curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1);
    	curl_easy_setopt(curl, CURLOPT_TCP_NODELAY, 1);
    	curl_easy_setopt(curl, CURLOPT_HTTPPROXYTUNNEL, 1);
    #if LIBCURL_VERSION_NUM >= 0x070f06
    ...
    #if LIBCURL_VERSION_NUM >= 0x071101
    	curl_easy_setopt(curl, CURLOPT_OPENSOCKETFUNCTION, opensocket_grab_cb);
    	curl_easy_setopt(curl, CURLOPT_OPENSOCKETDATA, &v->sock);
    #endif
    	curl_easy_setopt(curl, CURLOPT_CONNECT_ONLY, 1);
    ...
    #if LIBCURL_VERSION_NUM < 0x071101
    	/* CURLINFO_LASTSOCKET is broken on Win64; only use it as a last resort */
    	curl_easy_getinfo(curl, CURLINFO_LASTSOCKET, (long *)&v->sock);
    #endif
